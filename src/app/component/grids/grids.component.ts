...
export class GridsComponent implements OnChanges, AfterViewInit, OnDestroy {
  ...
  private refreshVirtualScroll(): void {
    if (this.isActive) {
      ...
      if (this.isVirtualScroll) {
        this.nzTableComponent?.cdkVirtualScrollViewport?.scrollToOffset(
          this.scrollPosition
        );
        // 强制重新计算视口大小
        ...
        const tableBody =
          this.el.nativeElement.querySelector(".ant-table-body");
        if (tableBody) {
          // 确保在DOM更新后再设置滚动位置
          setTimeout(() => {
  ...
  private getScrollPosition(): number {
    if (this.nzTableComponent?.cdkVirtualScrollViewport) {
      // 虚拟滚动情况下的滚动位置
      return (
        this.nzTableComponent.cdkVirtualScrollViewport.getOffsetToRenderedContentStart() ??
        0
      );
    } else {
      // 普通滚动情况下的滚动位置
  ...
  get isSelectAll() {
    return (
      this.setOfCheckedId.size === this.data.length &&
  ...
  private updateFiltersFn() {
    this.formGrid
      .filter((it) => it.isFilter)
      .forEach((it) => {
        it.filterFn = (list: string[], item: any) =>
          list.some((name) => item[it.code].indexOf(name) !== -1);
      });
  }
