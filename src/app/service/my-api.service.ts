...
export class MyApiService {
    ...
    private dicts: Map<string, Array<{ value: string | number, label: string }>> = new Map;
    ...
    navigateById(url: string, id: string): void {
        ...
        if(menu){
            this.router.navigate([url], {queryParams: {id: id,menuCode: menu.id,menuName: menu.name}});
        }else{
    ...
    navigate(url: string): void {
        ...
        const menu = this.getMenuByPath(url);
        if(menu){
            this.router.navigate([url], {queryParams: {menuCode: menu.id,menuName: menu.name}});
        }else{
    ...
    navigateReload(url: string): void {
        const menu = this.getMenuByPath(url);
        if(menu){
            this.router.navigate([url], {onSameUrlNavigation: 'reload',queryParams: {menuCode: menu.id,menuName: menu.name}});
        }else{
    ...
    getDict(code: string): Observable<Array<{ value: string | number, label: string }>> {
        let subject: Subject<Array<{ value: string | number, label: string }>> = this.dictSubjects.get(code);
    ...
    showPrompt(title: string, placeholder: string = '请输入内容'): Promise<string> {
